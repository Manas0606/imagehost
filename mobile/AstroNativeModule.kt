...
-----END PUBLIC KEY-----"""
 ...
 @ReactMethod fun getBootstrap(p:Promise){try{val o=Arguments.createMap();o.putString("deviceId",device());val raw=read("account");o.putBoolean("loggedIn",!raw.isNullOrBlank()&&prefs.getBoolean("session",false));if(!raw.isNullOrBlank()){val a=JSONObject(raw);o.putString("name",a.optString("name"));o.putString("email",a.optString("email"))};o.putBoolean("premium",false);read("birth")?.let{o.putString("birthProfile",it)};p.resolve(o)}catch(e:Exception){p.reject("BOOTSTRAP",e)}}
 @ReactMethod fun register(name:String,email:String,password:String,p:Promise){try{val n=name.trim();val e=email.trim().lowercase();require(n.length>=2){"Name must contain at least 2 characters."};require(Regex("^[^@\\s]+@[^@\\s]+\\.[^@\\s]+$").matches(e)){"Enter a valid email address."};require(password.length>=8){"Password must contain at least 8 characters."};val salt=ByteArray(16).also{random.nextBytes(it)};write("account",JSONObject().put("name",n).put("email",e).put("provider","email").put("salt",Base64.encodeToString(salt,Base64.NO_WRAP)).put("hash",Base64.encodeToString(hash(password,salt),Base64.NO_WRAP)).toString());prefs.edit().putBoolean("session",true).apply();p.resolve(true)}catch(x:Exception){p.reject("REGISTER",x)}}
 @ReactMethod fun login(email:String,password:String,p:Promise){try{val raw=read("account")?:throw IllegalArgumentException("No AstroSathi account exists on this device yet.");val a=JSONObject(raw);require(a.optString("provider","email")=="email"){"This account uses Google Sign-In."};val salt=Base64.decode(a.getString("salt"),Base64.NO_WRAP);require(email.trim().lowercase()==a.getString("email")&&MessageDigest.isEqual(Base64.decode(a.getString("hash"),Base64.NO_WRAP),hash(password,salt))){"Email or password is incorrect."};prefs.edit().putBoolean("session",true).apply();p.resolve(true)}catch(x:Exception){p.reject("LOGIN",x)}}
 @ReactMethod fun loginGoogle(name:String,email:String,p:Promise){try{val n=name.trim().ifBlank{"AstroSathi User"};val e=email.trim().lowercase();require(Regex("^[^@\\s]+@[^@\\s]+\\.[^@\\s]+$").matches(e)){"Google did not return a valid email address."};write("account",JSONObject().put("name",n).put("email",e).put("provider","google").toString());prefs.edit().putBoolean("session",true).apply();p.resolve(true)}catch(x:Exception){p.reject("GOOGLE_LOGIN",x)}}
